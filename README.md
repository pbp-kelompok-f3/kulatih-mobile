...
## Download
Download aplikasi versi terbaru: [Download APK](https://app.bitrise.io/app/4a391b08-240a-45d4-afc1-bcadff5d0ed5/installable-artifacts/5f2eab41f17b6e10/public-install-page/017db34249ed0c6c339a754c0cf38e21)
...
KuLatih adalah platform website dan mobile yang dirancang untuk menjembatani pelatih olahraga dengan individu atau kelompok yang membutuhkan bimbingan. Banyak pelatih memiliki kemampuan dan pengalaman luar biasa, tetapi kesulitan menjangkau calon murid karena promosi yang tersebar dan kurang terstruktur. Sementara itu, calon murid sering kesulitan menemukan pelatih yang sesuai dengan cabang olahraga yang diminati.
...
<details align="justify">
    <summary><b>📱 Integrasi Flutter dengan Web Service Django</b></summary>

Repository ini menjelaskan langkah-langkah melakukan integrasi antara aplikasi Flutter dengan web service Django yang sebelumnya telah dibuat pada Proyek Tengah Semester. Fokus utama integrasi ini adalah sistem autentikasi berbasis session cookie, sehingga aplikasi Flutter dapat melakukan login, logout, dan berkomunikasi dengan backend Django secara aman.

Pertama, buat sebuah django-app baru bernama authentication di dalam project Django yang sudah ada. Setelah itu, tambahkan app tersebut ke dalam bagian INSTALLED_APPS pada file settings.py.

Selanjutnya, instal library django-cors-headers untuk mengaktifkan komunikasi antara Flutter dan Django. Setelah instalasi, tambahkan library ini ke daftar INSTALLED_APPS, kemudian tambahkan juga middleware CorsMiddleware pada bagian paling atas daftar middleware Django.

Untuk mendukung proses autentikasi lintas platform, tambahkan pengaturan CORS dan konfigurasi cookie pada settings.py. Pengaturan ini mencakup mengizinkan seluruh origin, mengaktifkan kredensial, serta membuat cookie bersifat aman dan kompatibel dengan Flutter.

Jika menggunakan emulator Android, tambahkan alamat 10.0.2.2 ke dalam ALLOWED_HOSTS agar aplikasi Flutter dapat mengakses server Django yang berjalan pada komputer host.

Untuk mempermudah komunikasi antara Flutter dan Django, gunakan package pbp_django_auth yang telah disediakan. Package ini digunakan untuk melakukan proses HTTP GET dan POST sekaligus mengelola session cookie. Instal package tersebut melalui terminal proyek Flutter.

Agar package ini dapat digunakan di semua widget, sesuaikan root widget pada main.dart dengan menambahkan provider yang menyediakan instance CookieRequest untuk seluruh widget di aplikasi.

Selanjutnya, buat sebuah halaman baru bernama login.dart di folder screens. Halaman ini akan berfungsi sebagai interface login dan memanfaatkan fungsi-fungsi autentikasi dari pbp_django_auth.

Selain itu, instal juga package http karena akan digunakan untuk melakukan operasi HTTP tambahan yang mungkin diperlukan di luar autentikasi.

Agar aplikasi dapat mengakses internet, tambahkan izin akses internet pada file AndroidManifest.xml di dalam folder Android project Flutter.
